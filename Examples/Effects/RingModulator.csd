...
image pos(0, 0), size(435, 90), colour("black"), shape("rounded"), outlinecolour("lime"), outlinethickness(5) 
rslider bounds(  5, 12, 70, 70), text("Freq."),    channel("freq"),  range(0, 15000, 800, 0.25),   colour("yellow"),    trackercolour(white), textcolour(white)
rslider bounds( 75, 12, 70, 70), text("Sine/Sq."), channel("wave"),  range(0, 1.00, 0),            colour("yellow"),    trackercolour(white), textcolour(white)
line    bounds(150,  2, 3, 86), colour("lime")
rslider bounds(160, 12, 70, 70), text("Env."),     channel("env"),   range(0, 1.00, 0.5, 0.5),     colour("orange"),    trackercolour(white), textcolour(white)
rslider bounds(230,  6, 40, 40), text("Att."),     channel("att"),   range(0.001, 0.5, 0.01, 0.5), colour("orange"),    trackercolour(white), textcolour(white)
rslider bounds(230, 46, 40, 40), text("Dec."),     channel("dec"),   range(0.001, 0.5, 0.1,  0.5), colour("orange"),    trackercolour(white), textcolour(white)
line    bounds(275,  2, 3, 86), colour("lime")
rslider bounds(290, 12, 70, 70), text("Mix"),      channel("mix"),   range(0, 1.00, 1),            colour("chocolate"), trackercolour(white), textcolour(white)
rslider bounds(360, 12, 70, 70), text("Level"),    channel("level"), range(0, 1.00, 1),            colour("chocolate"), trackercolour(white), textcolour(white)
</Cabbage>
<CsoundSynthesizer>
<CsOptions>
-d -n
