...
image bounds(-640, 10, 200, 200), identchannel("drum4"), colour(150,150,170), shape("rounded"), outlinecolour("white"), line(0)
image bounds( -10,220, 200, 200), identchannel("drum5"), colour(150,150,170), shape("rounded"), outlinecolour("white"), line(0)
image bounds(-220,220, 200, 200), identchannel("drum6"), colour(150,150,170), shape("rounded"), outlinecolour("white"), line(0)
...
;modification of Iain's original instument so that it adapts to the Android 
;screen size
instr 1
...
iPadHeight = iHeight/2.4

kCnt init 0

if kCnt<4 then
	SMessage sprintfk "bounds(%d, 10, %d, %d)", 10+(kCnt*(iPadWidth+20)), iPadWidth, iPadHeight
	SChannel sprintfk "drum%d", kCnt+1
	chnset SMessage, SChannel
	kCnt=kCnt+1
elseif kCnt<8 then
	SMessage sprintfk "bounds(%d, %d, %d, %d)", 10+((kCnt-4)*(iPadWidth+20)), iPadHeight+30, iPadWidth, iPadHeight
	SChannel sprintfk "drum%d", kCnt+1
	chnset SMessage, SChannel
	kCnt=kCnt+1
endif
...
<CsScore>
i1 0 1
i 10 1 3600	;INSTRUMENT 1 PLAYS A NOTE FOR 1 HOUR (AND KEEPS REAL-TIME PERFORMANCE GOING)
</CsScore>
