...
<Cabbage>
form caption("Pebbles in a Pond"), scrollbars(0), size(600, 600), pluginID("Pond"),colour(0,0,0), guirefresh(128)
image bounds(0,0,0,0), colour(0,0,0), channelarray("circle",70), shape("ellipse"), outlinecolour("white"), line(0)
label    bounds(  0, 245,600, 16), text("Drop a pebble into the pond..."), fontcolour(white), identchannel("instructions")
</Cabbage>
...
ksmps 		= 	16	;NUMBER OF AUDIO SAMPLES IN EACH CONTROL CYCLE (higher values will cause quantisation noise in the mode filter)
nchnls 		= 	2	;NUMBER OF CHANNELS (2=STEREO)
0dbfs		=	1

gisine	ftgen	0,0,131072,10,1
giPanelSizeX	=	600		; Should correspond to the size of the Cabbage panel
giPanelSizeY	=	600		; Should correspond to the size of the Cabbage panel
...
instr	2	; Draw an expanding circle and create a 'plip' sound 
 ...
 iRed		random	0,255				; Colours are set randomly once per event
 iGreen		random	0,255				; Colours are set randomly once per event
 iBlue		random	0,255				; Colours are set randomly once per event
 kAlpha	transeg	255,0.4,0,100 ,p3-0.5,-3, 0,1,0,0	; Colour alpha decreases as event progresses, i.e. transparency increases
 
 gkRefreshTrig	metro	kr/128				; Create a metronome which will define the rate of graphical update of the expanding circles
 ...
 iStartCPS	scale_i	(iXNorm+(1-iYNorm))*0.5,700,30		; 'Plip' starting CPS varies according to mouse X position and mouse Y position.
 iEndCPS	scale_i	1-iYNorm,14000,800	; 'Plip' ending CPS varies according to mouse Y position (inverted).
...
instr	1000	; Print initial text instructions, expanding and disappearing.
 ...
 kAlpha		transeg	255,p3-0.1,-4,0,0.1,0,0		; Colour alpha decreases as event progresses, i.e. transparency increases
 ...
 if ktrig==1 then				; If a trigger has been generated...
  ...
  UPDATE_TEXT:				; A label. Begin reinitialisation from here.
   Smessage1 sprintfk "bounds(-200, 245,1000, %d),fontcolour(255,255,255,%d)", i(kSize), i(kAlpha)	; Create a string with updated attributes that will be sent to the instructions widget.
