...
void CabbagePluginListComponent::removeSelected()
{
...
void CabbagePluginListComponent::buttonClicked (Button* button)
{
...
{
public:
    Scanner (CabbagePluginListComponent& plc, AudioPluginFormat& format, PropertiesFile* properties, int threads)
          ...
          progress (0.0), numThreads (threads), finished (false)
    {
        ...
        if (path.getNumPaths() > 0) // if the path is empty, then paths aren't used for this format.
        {
            ...
            pathList.setColour(FileSearchPathListComponent::backgroundColourId, cUtils::getDarkerBackgroundSkin());

            pathChooserWindow.setColour(ListBox::textColourId, Colours::red);
