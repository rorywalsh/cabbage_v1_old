...
; 'Record' plays and simultaneously records the time-stretched. 
;    The output file will be created in the same directory as the input file and with the same name but with the word '_stretched' suffixed. 
...
<Cabbage>
form caption("File Stretcher") size(580,230), colour(0,0,0) pluginID("FiSt"), guirefresh(10) 
...
button     bounds( 95,160, 80, 25), text("PLAY","PLAY"), channel("Play"), value(0), latched(0), fontcolour(100,255,100)
button     bounds( 95,190, 80, 25), text("STOP","STOP"), channel("Stop"), value(0), latched(0), fontcolour(150,150,255)
...
instr	1
 ...
 ktrig	trigger	gkPlay,0.5,0			; Generate an impulse if PLAY button is clicked
 schedkwhen	ktrig,0,0,2,0,0.01,0

 ktrig	trigger	gkRecord,0.5,0			; Generate an impulse if PLAY button is clicked
 schedkwhen	ktrig,0,0,2,0,0.1, 1
...
instr	2
 ...
 if ktrig==1 then		
  ...
  SOutputName	strcat	SOutputName,"_stretched.wav"	; Add suffix
  ...
  if gichans=1 then						; Mono
   a1	mincer		aptr+ajit, 1, 1, gitableL, gklock, iFFTSize
  	outs	a1*gkLevel,a1*gkLevel
  	if p4==1 then
  	 fout	SOutputName,4,a1
  	endif

  elseif gichans=2 then						; Stereo
   a1	mincer		aptr+ajit, 1, 1, gitableL, gklock, iFFTSize
   a2	mincer		aptr+ajit, 1, 1, gitableR, gklock, iFFTSize
  	outs	a1*gkLevel,a2*gkLevel
  	if p4==1 then
  	 fout	SOutputName,4,a1,a2
  	endif
  endif
 endif
endin
