<Cabbage>
form caption("Inharmonic Synth") size(460, 310)
...
groupbox bounds(10, 10, 260, 90), text("Filter Envelope"), plant("Filter Envelope"){
rslider pos(0, 25), size(60,60), text("Amount"), colour("cornflowerblue"), channel("FAmt"), range(0, 1, 0.7)
rslider pos(50, 25), size(60,60), text("Dec."), colour("cornflowerblue"), channel("FDec"), range(0, 1, 0.45)
rslider pos(100, 25), size(60,60), text("Sus."), colour("cornflowerblue"), channel("FSus"), range(0, 1, 0)
rslider pos(150, 25), size(60,60), text("Rel."), colour("cornflowerblue"), channel("FRel"), range(0, 1, 0.45)
rslider pos(200, 25), size(60,60), text("Shape"), channel("FShp"), colour("cornflowerblue"), range(-50, 50, -16)
}

groupbox bounds(10, 105, 210, 90), text("Amplitude Envelope"), plant("Amplitude Envelope"){
rslider pos(0, 25), size(60,60), text("Att."), channel("AAtt"), colour("red"), range(0, 1, 0)
rslider pos(50, 25), size(60,60), text("Dec."), channel("ADec"), colour("red"),range(0, 1, 0.45)
rslider pos(100, 25), size(60,60), text("Sus"), channel("ASus"), colour("red"),range(0, 1, 0)
rslider pos(150, 25), size(60,60), text("Rel"), channel("ARel"), colour("red"),range(0, 1, 0.45)
}

groupbox bounds(225, 105, 210, 90), text("Chorus"), plant("Chorus"){
checkbox channel("ChoOnOff"), pos(10, 30), size(50, 50), value(1), text("On/Off")
rslider pos(50, 25), size(60,60), text("Speed"), channel("ChoSpeed"), range(0, 1, 0.6)
rslider pos(100, 25), size(60,60), text("Width"), channel("ChoWidth"), range(0, 1, 1)
rslider pos(150, 25), size(60,60), text("Mix"), channel("ChoMix"), range(0, 1, 0.5)
}
...
instr	2	;SOUND GENERATING INSTRUMENT
	...
	kspeed	chnget	"ChoSpeed"
	kwidth	chnget	"ChoWidth"
	kwidth	=	1-kwidth
	kchomix	chnget	"ChoMix"
	;STEREO CHORUS----------------------------------------------------------
	if kChoOnOff==1 then
	  adly1		oscili	1,kspeed,gilfoshape				;CHORUS DELAY TIME LFO 1
	  adly2		oscili	1,kspeed,gilfoshape,0.5				;CHORUS DELAY TIME LFO 2
	  ...
	  aL		sum	asig, acho1*kchomix, acho2*kchomix*kwidth 	;CREATE LEFT CHANNEL MIX
	  aR		sum	asig, acho1*kchomix*kwidth, acho2*kchomix 	;CREATE RIGHT CHANNEL MIX
	  ;SEND STEREO AUDIO TO OUTPUTS
	  		outs	acho1*kchomix + acho2*kchomix*kwidth, acho1*kchomix*kwidth + acho2*kchomix
	endif
	;-----------------------------------------------------------------------
